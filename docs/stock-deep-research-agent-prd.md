...
> 一个面向**非专业用户**的对话式美股研究助手。用户用自然语言说"研究哪几家公司、看哪段时间、关注什么"，Agent 自动识别公司、获取行情、确定性计算市场风险、（核心加分）围绕异动检索事件证据、横向比较，最终产出一份**可下载的英文研究报告**。
...
| 层 | 内容 | 定位 |
|---|---|---|
| **A · 核心** | 识别公司 → 定时间 → 取行情 → 代码算市场风险指标 → 归一化对比 → 可下载报告 | 面试官明确要求 |
| **B · 加分（演示核心，必须交付）** | 找异动 → Tavily 查事件 → Event Attribution Confidence → Short-term Market View | 面试官没要求，但**主演示问题"为什么涨跌不同"靠它**，所以 B 不是可砍项，第二天必须稳交付 |
| **B+ · best-effort 深度** | Business Risks（从 10-K Item 1A / 财报提取经营风险）| 有亮点但较重，做不动就降级为"基于公开新闻的经营风险提示"，不阻塞 A |
| **轻量核心 · 上传文件** | PDF/TXT/MD **文本提取** → 归属公司 → 织进报告并标**文件名+页码**（**不做 OCR / 向量库**）| 纳入本期；非必填，A+B 稳定后接入（见 §16）|
| **明确不做** | 账号/登录/跨会话历史、自动交易、目标价/估值/DCF、A 股/港股/加密、OCR、向量库/复杂 RAG、多 Agent 辩论、单次 >3 只 | 未要求或拖垮三天稳定性 |
...
### 指标口径（明确定义）
- **复权收盘价**：本期**默认用拆股复权日收盘价**（Twelve Data 日线默认拆股复权 ✅）；**数据源若同时提供分红复权价则优先用**；数据源无法提供任何复权价时，降级用普通收盘价并在报告中明确标注（避免因无复权数据导致整任务失败）。**未含分红调整时，区间收益称为 Price Return，不称 Total Return。**
- **区间收益率** = 期末收盘 / 期初收盘 − 1。
...
> 把"风险"拆成三块独立输出，再单独给一个"短期市场观点"。模型只解释/提取，**不算指标、不编造概率**。配置化阈值**不对外宣称是行业标准**；演示用 NVDA/BABA/INTC 做结果检查，**不当作正式校准**（正式校准需更广样本）。
...
> **三天范围**：完整 10-K 抽取较重 → 本期 best-effort（拉最新 10-K 的 Item 1A，模型摘 top 3–5 类 + 标来源）；做不动就降级为"基于公开新闻的经营风险提示"。**不阻塞 A 核心**。
...
前端让用户看到 Agent 如何研究（不是一次性吐答案）。**研究计划展示后自动执行**（不设人工确认门槛，演示连贯）。
...
记住：当前公司、时间范围、关注点、各股**结构化研究结果**、已获取证据、当前报告。
> 重点不是"聊天记录"，而是 **Agent 知道哪些数据查过、哪些要重查**。第一问出报告，之后基于已分析内容继续答，不是每轮都出整份报告。

### 追问三类
- **只改分析角度**（"重点看风险"）→ 复用已有结果重新组织，不重新取数。
- **改公司 / 时间范围** → 重新执行研究。
- **问缺失信息**（"英伟达那段为什么跌"且没查过）→ 只执行对应补充步骤。

三天版本只做**一个当前研究会话**；刷新/关闭后是否保留**不作为核心验收**。
...
7. **Business Risks**（分类提取 + 来源标注；"Catalysts" 本期不单独定义）
...
| 事件方向打平/混合 | 记 unclear |
| 上传文件与公开信息冲突（P1）| 报告明确提示冲突，不用文件覆盖行情 |
| 某个 Research Agent 超时 | 其他任务继续；报告说明缺失部分 |
| 数据来源时间不同 | 分别展示行情截止时间与事件数据时间 |

> **缓存/离线快照**：本期**不作硬性要求**——优先把在线调用调稳；若演示当场失败，如实说明并展示降级，而不是伪装成实时。（如有余力可准备带时间戳快照，但不强制。）

---

## 15. 三天交付安排

- **第一天 · 单股闭环**：识别→行情→市场风险指标→（B）异动+事件→单股结论。验收：输入"分析英伟达最近三个月，解释明显涨跌原因" → 正确身份/标的类型、市场风险指标、最大单日/最大回撤、带来源事件 + Event Attribution Confidence、Short-term Market View。
- **第二天 · 多股并行 + 报告（含 B）**：并行研究、可见状态、横向比较与排名、可下载报告、当前会话追问。验收：输入主演示问题 → 并行计划与状态 / 三股研究 / 横向比较 / 生成并下载英文报告。**B 必须在此稳定（演示靠它）。**
- **第三天 · 稳定与交付**：验证固定成功场景 / 行情失败 / 新闻不足 / 单任务超时；完善来源时间与免责声明；整理 README + 演示讲稿；至少两次计时彩排。上传文件（轻量核心）在主链路稳定后接入；B+ Business Risks 时间不够可降级为"基于公开新闻的经营风险提示"。
...
> 纳入本期的**轻量核心**能力：只做 PDF/TXT/MD **文本提取**，**不做 OCR、不做向量库**。不改核心流程（叠加，不是重构），没上传也能完成完整研究；A+B 主链路稳定后接入。
...
**演示稳定性**：固定三股（NVDA/BABA/INTC）能连续成功两次 / 一次演示 ≤ 5 分钟 / 外部失败时清晰降级或如实说明 / 备固定问题 + 期望事实样本集。

---

## 18. 后续开发委派说明

本文件是后续开发的需求基线。每个任务含：用户场景、输入与期望输出、涉及阶段、前端需展示状态、可验证验收示例、必须运行的验证。委派以**可演示业务闭环**为单位，按纵向链路推进。
