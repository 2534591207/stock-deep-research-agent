...
> 版本：对齐版 v2（2026-06-07）。配合 PRD `docs/stock-deep-research-agent-prd.md`。
> 三个**免费**数据源，对应 PRD 的能力分层：行情（Twelve Data，A 核心）· 新闻事件（Tavily，B 加分）· 财务事实（SEC EDGAR，可选核验）。
...
| 新闻 / 事件 / 舆情 | **Tavily** | 可按公司 + 日期窗口检索，返回标题、链接、时间、摘要 | B 加分 |
| 财务事实、最新申报记录 | **SEC EDGAR** | 官方、免费、不需要 API Key | 可选核验 |

> **上传文件（轻量核心）**：本期支持 PDF/TXT/MD **文本提取**作为补充证据（不做 OCR/向量库，见 PRD §16）；由**本地解析**处理，非外部 API。
