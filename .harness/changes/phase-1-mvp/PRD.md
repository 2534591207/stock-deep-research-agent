...
> 边界：本期**仅当前内存会话**，不承诺刷新/重启恢复；**报告版本是当前会话内版本，不是账号历史**；上传文件按**页/分块索引**引用，**不建向量库**。
...
### 语言与格式
**英文**正文；应用内 HTML 渲染（含走势图）；**本期下载仅 Markdown**（图用静态 PNG 嵌入）。**PDF 为后续（前端/导出）阶段，不在本期后端范围。**
...
**报告**：英文多股报告 / 每股 9 节含标的类型 / 含走势、市场风险、事件、经营风险（best-effort）、Short-term Market View、来源 / 可下载（**Markdown**；PDF 后续）/ 始终含免责声明 / 相对排名标注"仅限本次所选股票与区间"。
